...
def extract_text_from_pdf(content: bytes) -> str:
    """Sync function to extract text from PDF bytes."""
...
async def process_question_task(lecture_id: str, slide_url: Optional[str]):
    """Background task to download PDF, extract text, generate question and push via WebSocket."""
        ...
        try:
            ...
            if file_id:
                async with httpx.AsyncClient() as client:
                    resp = await client.get(f"https://drive.google.com/uc?export=download&id={file_id}", timeout=20.0)
                    if resp.status_code == 200:
                        # Offload blocking PDF extraction to a thread
                        slide_text = await anyio.to_thread.run_sync(extract_text_from_pdf, resp.content)
